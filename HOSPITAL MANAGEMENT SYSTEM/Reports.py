...
class REPORTS:
    def _reports_(self):
        ...
        global  tree_data
        self.tree_data=ttk.Treeview(self.data_frame,columns=("ReportID",'PurID','Date'),yscrollcommand=self.scrollbar.set)
        self.scrollbar.config(command=self.tree_data.yview)
        self.scrollbar.pack(fill=Y,side=RIGHT)
        self.tree_data.column('#0',width=0,stretch=NO)
        self.tree_data.column("ReportID",width=120)
        self.tree_data.column("PurID",width=200)
        self.tree_data.column("Date",width=240)
        self.tree_data.heading("ReportID",text='ReportID')
        self.tree_data.heading("PurID",text='PurchaseID')
        self.tree_data.heading("Date",text='Date')
