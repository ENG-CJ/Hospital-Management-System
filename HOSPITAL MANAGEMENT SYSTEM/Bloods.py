...
class BLOODS:
    def blood(self):
        ...
        self.Delete=Button(self.screen,text='Delete',bg='#189',fg='white',font=('Verdana', 16, 'bold'),
                      bd=3,relief='sunken',command=self.delete)
        ...
        global  tree_data
        self.tree_data=ttk.Treeview(self.data_frame,columns=("BloodID",'Blood-type'),yscrollcommand=self.scrollbar.set)
        self.scrollbar.config(command=self.tree_data.yview)
        self.scrollbar.pack(fill=Y,side=RIGHT)
        self.tree_data.column('#0',width=0,stretch=NO)
        self.tree_data.column("BloodID")
        self.tree_data.column("Blood-type")
        self.tree_data.heading("BloodID",text='BloodID')
        self.tree_data.heading("Blood-type",text='Blood-Type')
